...
jq(document).ready( function() {
	...
	*/
	...
	jq(".aw-whats-new-submit").click( function() {
		...
		console.log(content +'-'+item_id+'-'+content);
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				jq("#activity-stream div:first").addClass('new-update');



				if ( 0 != jq("#latest-update").length ) {

					var l = jq("#activity-stream .new-update .activity-content .activity-inner p").html();

					var v = jq("#activity-stream .new-update .activity-content .activity-header p a.view").attr('href');



					var ltext = jq("#activity-stream .new-update .activity-content .activity-inner p").text();
				...
				jq(".new-update").hide().slideDown( 300 );

				jq(".new-update").removeClass( 'new-update' );
...
*/
	...
	jq('.activity-read-more a').on('click', function(event) {
		...
		var selector = '#' + type + '-' + a_id + ' .' + inner_class;

		var a_inner = jq(selector);
		...
		if ( target.attr('name') == 'ac_form_submit' ) {
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] == '-1' ) {
					...
					jq('#activity-' + form_id[2] + ' a.acomment-reply span').html( Number( jq('li#activity-' + form_id[2] + ' a.acomment-reply span').html() ) + 1 );
